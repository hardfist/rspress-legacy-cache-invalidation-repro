...
的 job summary，原始 JSON 会作为 artifact 上传。为临时绕过该 canary 在 Linux 上
第二次 reopen 时可能丢失 SST 文件的问题，每个 warm/touch 样本只复用一次独立的冷缓存。
...
Rspress 把一个 `file:` URL 作为 build dependency 传给 Rspack，而 Rspack legacy
cache 把所有 build dependency 都当成普通文件路径执行 `path.resolve`。最终得到
一个不存在的路径，snapshot 校验又把该路径的 `Strategy::Failed` 固定视为
`Modified`，导致每个新进程都执行全量缓存重置。
